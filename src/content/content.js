(() => {
  ...
  function renderStory(r) {
    ...
    // Treść niezwiązana z profilem: model zwraca relevant=false i pustą listę
    // porad. Zamiast naciąganych „takeawayów" pokazujemy uczciwą notkę.
    if (r.relevant === false || (!(r.takeaways || []).length && !(r.read_next || []).length)) {
      const note = r.lesson
        ? mdBold(r.lesson)
        : escapeHtml(tr("careerOffTopic"));
      setBody(
        `<div class="krytykai-meta">${badge}</div>` +
          `<div class="krytykai-offtopic">🤷 ${note}</div>`
      );
      return;
    }
  ...
  async function runComments() {
    ...
    try {
      ...
      renderComments(resp.result, batch, dropped, items.length);
    } catch (err) {
      const msg = String(err?.message || err);
      if (isContextInvalidated(msg)) renderContextInvalidated();
      else renderError(msg);
  ...
  function renderComments(result, batch, dropped, totalSeen) {
    showPanel();
    ...
    // Wygaszamy TYLKO to, co odsiało sito lokalne (sam emoji, wyzwisko bez
    // treści, krzyk bez argumentu) – kryteria obiektywne i tanie w sprawdzeniu.
    // Listy „empty" od modelu świadomie do tego nie używamy: na prawdziwym
    // wątku HN uznał za puste m.in. trafną anegdotę i pytanie techniczne,
    // a ukrycie dobrego komentarza jest gorsze niż jego niewyróżnienie.
    for (const c of dropped) {
      if (c.el.classList.contains("krytykai-c-top")) continue;
