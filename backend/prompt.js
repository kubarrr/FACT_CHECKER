...
export function buildSystemPrompt(numQuestions, language) {
  ...
  return [
    ...
    "ideology, social debates, war, contested economics): even when the facts look accurate and",
    "risk is LOW, the framing can still be ONE-SIDED. In such cases you MUST include, among the",
    `${numQuestions} questions:`,
    '   - at least ONE "perspective" question (kind: "perspective") that surfaces the OTHER SIDE:',
    "     what critics or political opponents say, alternative interpretations, or whether the",
    "     framing/selection is one-sided. Stay STRICTLY NEUTRAL — do NOT take a political side.",
...
export function buildStorySystemPrompt(language, skillOptions) {
  ...
  return [
    ...
    "",
    'RELEVANCE — judge it generously against the user\'s ROLE and SKILLS, not only their exact',
    "goal or industry. Content is RELEVANT when it shares a method, technology, tool, concept or",
    "transferable idea with their work — even in a DIFFERENT domain. For a data analyst, an",
    "article about data analysis in agriculture, sports or retail is relevant: same craft, new",
    "setting. It does NOT have to match their specific project (e.g. an EEG startup) to count;",
    "a useful technique or an idea they could reuse is enough.",
    "",
